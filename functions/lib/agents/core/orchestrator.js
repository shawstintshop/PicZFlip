    ...
    async processPhotoAnalysis(imageBuffer, uid, analysisId) {
        ...
        try {
            ...
            analysis = await this.runStage('searching', async () => {
                const searchResults = await searchManager.executeSearchPlan(analysis.routing.searchPlan, analysis.item);
                return {
                    ...analysis,
                    status: 'searching',
                    searchResults,
            ...
            analysis = await this.runStage('aggregating', async () => {
                const aggregatedData = await resultsAggregator.processSearchResults(analysis.searchResults, analysis.item);
                return {
                    ...analysis,
                    status: 'aggregating',
                    aggregatedData,
            ...
            analysis = await this.runStage('copywriting', async () => {
                const copyData = await copywriterAgent.generateListingCopy(analysis.item, analysis.pricing);
                return {
                    ...analysis,
                    status: 'copywriting',
                    copy: copyData,
