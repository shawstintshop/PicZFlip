export class BaseAdapter {
    ...
    parseCondition(conditionText) {
        ...
        if (text.includes('like new') || text.includes('excellent'))
            return 'like_new';
        if (text.includes('good') || text.includes('very good'))
            return 'good';
        if (text.includes('fair') || text.includes('acceptable'))
            return 'fair';
        if (text.includes('poor') || text.includes('damaged'))
            return 'poor';
        if (text.includes('refurbished') || text.includes('reconditioned'))
            return 'refurbished';
        return 'unknown';
    }
    /**
    ...
    parseShipping(shippingText) {
        ...
        if (text.includes('free') || text.includes('no charge'))
            return 'free';
        if (text.includes('calculated') || text.includes('varies'))
            return 'calculated';
        if (text.includes('pickup') || text.includes('local'))
            return 'local_pickup';
    ...
    normalizeCurrency(currencyText) {
        ...
        if (text.includes('USD') || text.includes('$') || text.includes('DOLLAR'))
            return 'USD';
        if (text.includes('EUR') || text.includes('€') || text.includes('EURO'))
            return 'EUR';
        if (text.includes('GBP') || text.includes('£') || text.includes('POUND'))
            return 'GBP';
        if (text.includes('CAD') || text.includes('C$'))
            return 'CAD';
        if (text.includes('AUD') || text.includes('A$'))
            return 'AUD';
     ...
     */
    generateSearchQueries(itemData) {
        const queries = [];
        if (itemData.name) {
     ...
     * Log search attempt
     */
    logSearch(sourceId, query, resultCount) {
