...
export const analyzePhoto = onCall(async (data, context) => {
    if (!context.auth) {
...
export const getAnalysis = onCall(async (data, context) => {
    if (!context.auth) {
        throw new HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
async function enhancedPhotoAnalysis(imageBuffer, uid, analysisId, userContext) {
    ...
    try {
        ...
        const listingPlatforms = ['ebay', 'amazon', 'facebook'];
        const listings = {};
...
export const analyzePhotoEnhanced = onCall(async (data, context) => {
    if (!context.auth) {
        throw new HttpsError('unauthenticated', 'User must be authenticated');
    }
