...
export function validateEnvironment(): void {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing required environment variables: ${missing.join(
        ', '
      )}. Set FIREBASE_PROJECT_ID locally or run \`gcloud config set project <project-id>\` before deploying.`
    );
  }

  const hasVisionCredentials = Boolean(
    ENV.GOOGLE_VISION_API_KEY || ENV.GOOGLE_APPLICATION_CREDENTIALS
  );
  ...
  if (!hasVisionCredentials && !runningOnGoogleCloud) {
    throw new Error(
      'Vision API credentials missing. Provide GOOGLE_VISION_API_KEY or GOOGLE_APPLICATION_CREDENTIALS when running locally.'
    );
  }
