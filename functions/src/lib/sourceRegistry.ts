...
export function getSourcesByCategory(categories: string[], region?: string): MarketplaceSource[] {
...
export function getAllActiveSources(): MarketplaceSource[] {
  return MARKETPLACE_SOURCES.filter(source => source.active);
}

export function getSourceById(id: string): MarketplaceSource | undefined {
  return MARKETPLACE_SOURCES.find(source => source.id === id);
}

export function getSourcesByPriority(minPriority: number): MarketplaceSource[] {
  return MARKETPLACE_SOURCES
    .filter(source => source.active && source.priority >= minPriority)
    .sort((a, b) => b.priority - a.priority);
}

export function getSourcesByRegion(region: string): MarketplaceSource[] {
  return MARKETPLACE_SOURCES
